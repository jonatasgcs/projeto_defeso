...
if (isset($_GET['sair'])) {
    ...
    header("Location: admin_simulacoes.php");
...
if (isset($_GET['excluir'])) {
    ...
    // Query SQL para deletar uma simulação pelo ID.
    $sql_delete = "DELETE FROM simulacoes WHERE id = $id";
    if ($conn->query($sql_delete)) {
        $status_type = 'sucesso';
        $status_message = "Simulação excluída com sucesso.";
    } else {
        $status_type = 'erro';
        $status_message = "Erro ao excluir simulação: " . $conn->error;
    }
    // Redireciona para a mesma página para limpar o GET e evitar re-exclusão ao recarregar.
    header("Location: admin_simulacoes.php?status={$status_type}&mensagem=" . urlencode($status_message));
    exit;
}
...
if (isset($_POST['editar'])) {
    ...
    // Query SQL para atualizar os campos da simulação.
    $sql_update = "UPDATE simulacoes SET nome='$nome', email='$email', resultado='$resultado' WHERE id=$id";

    if ($conn->query($sql_update)) {
        $status_type = 'sucesso';
        $status_message = "Simulação atualizada com sucesso.";
    } else {
        $status_type = 'erro';
        $status_message = "Erro ao atualizar simulação: " . $conn->error;
    }
...
// Inicializa a variável $simulacao_para_editar como nula. Ela armazenará os dados de uma simulação específica se encontrada.
$simulacao_para_editar = null;
...
if (isset($_POST['buscar_id_para_editar'])) {
    ...
    // Query SQL para selecionar uma simulação específica pelo 'id'.
    $sql_select_one = "SELECT * FROM simulacoes WHERE id = $idBusca";
    $resultado_query = $conn->query($sql_select_one);
    // Verifica se a query retornou algum resultado.
    if ($resultado_query->num_rows > 0) {
        // Se uma simulação for encontrada, armazena seus dados na variável $simulacao_para_editar.
        $simulacao_para_editar = $resultado_query->fetch_assoc();
    } else {
        // Se nenhuma simulação for encontrada, define uma mensagem de erro.
        $erro = "Nenhuma simulação encontrada com ID $idBusca para edição.";
    }
}

// === LÓGICA PARA LISTAR SIMULAÇÕES (SELECT ALL) ===
// Inicializa o array para armazenar a lista de simulações.
$lista_simulacoes = [];
// Esta condição verifica se o botão 'listar_simulacoes' foi clicado OU se a página foi carregada sem uma ação específica de POST (editar, excluir, buscar_id_para_editar).
// Isso garante que a lista de simulações seja exibida por padrão ao carregar a página.
if (isset($_POST['listar_simulacoes']) || (!isset($_POST['editar']) && !isset($_GET['excluir']) && !isset($_POST['buscar_id_para_editar']))) {
    // Query SQL para selecionar todas as simulações, ordenadas pelo ID em ordem decrescente (as mais recentes primeiro).
    $sqlTodos = "SELECT id, nome, email, resultado FROM simulacoes ORDER BY id DESC";
    $resultadoTodos = $conn->query($sqlTodos);
    // Verifica se a query retornou resultados.
    if ($resultadoTodos->num_rows > 0) {
        // Itera sobre cada linha do resultado e adiciona-a ao array $lista_simulacoes.
        while($row = $resultadoTodos->fetch_assoc()) {
            $lista_simulacoes[] = $row;
        }
    }
}
...
<head>
    ...
    <style>
        ...
        .edit-form-container select { /* Adicionado select para o formulário de edição */
            width: 100%;
            padding: 0.75rem;
            margin-bottom: 1.5rem;
            border: 1px solid #CBD5E1;
        ...
        .data-table {
            width: 100%;
            min-width: 600px; /* Define uma largura mínima para a tabela para forçar a rolagem */
    ...
    <?php else: // Se o administrador ESTIVER logado, exibe o painel de administração ?>
        <div class="admin-section-block">
            <div class="admin-header">
                <h2>Administração de Simulações</h2>
                <a href="?sair=1" class="logout-btn"><i class="bi bi-box-arrow-right"></i> Sair</a>
            </div>

            <form method="POST" class="search-form">
                <label for="buscar_id_para_editar">Buscar Simulação pelo ID:</label>
                <div class="flex items-center">
                    <input type="number" name="buscar_id_para_editar" id="buscar_id_para_editar" class="flex-grow" placeholder="Digite o ID da simulação">
            ...
            <?php if ($simulacao_para_editar): // Exibe o formulário de edição se uma simulação foi buscada ?>
                <div class="edit-form-container">
                    <h3>Editar Simulação (ID <?= $simulacao_para_editar['id'] ?>)</h3>
                    <form method="POST">
                        <input type="hidden" name="id" value="<?= $simulacao_para_editar['id'] ?>">
                        <label for="nome">Nome:</label>
                        <input type="text" name="nome" id="nome" value="<?= htmlspecialchars($simulacao_para_editar['nome']) ?>" required>
                        <label for="email">Email:</label>
                        <input type="email" name="email" id="email" value="<?= htmlspecialchars($simulacao_para_editar['email']) ?>" required>
                        <label for="resultado">Resultado:</label>
                        <select name="resultado" id="resultado" required>
                            <option value="Sim" <?= ($simulacao_para_editar['resultado'] == 'Sim') ? 'selected' : '' ?>>Sim</option>
                            <option value="Não" <?= ($simulacao_para_editar['resultado'] == 'Não') ? 'selected' : '' ?>>Não</option>
                        </select>
                        <button name="editar" type="submit">Atualizar</button>
                        <a href="?excluir=<?= $simulacao_para_editar['id'] ?>" onclick="return confirm('Deseja realmente excluir esta simulação?')" class="delete-link"><i class="bi bi-trash"></i> Excluir</a>
                    </form>
                </div>
            <?php endif; ?>
            ...
            <div class="data-table-wrapper"> <table class="data-table">
                    ...
                    <tbody>
                    <?php if (!empty($lista_simulacoes)): ?>
                        <?php foreach ($lista_simulacoes as $row): ?>
                            <tr>
                                <td><?= $row['id'] ?></td>
                                ...
                                <td class="flex flex-col sm:flex-row gap-2">
                                    ...
                                    <form method="get" class="inline-block"> <input type="hidden" name="excluir" value="<?= $row['id'] ?>">
                                        <button type="submit" onclick="return confirm('Deseja realmente excluir esta simulação?')" class="table-action-btn delete"><i class="bi bi-trash"></i> Excluir</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr><td colspan="5" class="text-center py-4">Nenhuma simulação listada. Clique no botão "Listar Todas as Simulações" para exibir.</td></tr>
                    <?php endif; ?>
                    </tbody>
                </table>
            </div> <div class="reload-button-container">
                <form method="post">
                    <button type="submit" name="listar_simulacoes"><i class="bi bi-arrow-clockwise"></i> Listar Todas as Simulações</button>
                </form>
            </div>
        </div>
    <?php endif; ?>
